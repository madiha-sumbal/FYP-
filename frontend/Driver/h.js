...
import { 
  ...
  Dimensions, TextInput, ActivityIndicator, SafeAreaView 
} from "react-native";
...
const UnifiedDriverDashboard = ({ navigation, route }) => {
  ...
  const [dashboardStats, setDashboardStats] = useState({
    completedTrips: 0, activeTrips: 0, pendingTrips: 0, monthlyEarnings: 0
  });
  ...
  const [trips, setTrips] = useState([]);           // Trip History
  ...
  const loadAuthData = async () => {
    try {
      ...
      if (userDataStr) setDriverProfile(JSON.parse(userDataStr));
  ...
  // ==================== API HELPER ====================
  const getHeaders = useCallback((token = null) => ({
    'Content-Type': 'application/json',
    'Authorization': `Bearer ${token || authTokenRef.current || authToken}`
  }), [authToken]);

  // ==================== INITIAL LOAD ====================
  const loadAllData = useCallback(async (userId, token) => {
    setLoading(true);
    try {
      await Promise.all([
  ...
  // History tab pe auto refresh
  useEffect(() => {
    if (currentView === "History") fetchTrips();
  }, [currentView]);

  // ==================== FETCH FUNCTIONS ====================
  const fetchDashboardStats = useCallback(async (userId = null, token = null) => {
    try {
  ...
  const fetchCurrentTrip = useCallback(async (userId = null, token = null) => {
    try {
      const uid = userId || driverIdRef.current;
      ...
      if (data.success) {
        ...
        const activeTrip = myTrips.find(t => 
          ['Scheduled', 'En Route', 'Ready'].includes(t.status)
        );
        if (activeTrip && JSON.stringify(activeTrip) !== JSON.stringify(currentTrip)) {
          setCurrentTrip(activeTrip);
          if (activeTrip.status === 'En Route') setRouteStarted(true);
        }
      }
    } catch (error) {
      console.error('❌ Error fetching current trip:', error);
    }
  }, [currentTrip, getHeaders]);

  const fetchAvailabilityHistory = useCallback(async (userId = null, token = null) => {
    try {
      ...
      if (data.success) {
        ...
        if (activeRecords.length > 0) {
          ...
          if (now.getHours() >= 18 && now.getHours() < 21) setShowAvailabilityAlert(true);
        }
      }
    } catch (error) {
  ...
  const fetchTrips = useCallback(async (userId = null, token = null) => {
    try {
      const uid = userId || driverIdRef.current;
      const response = await fetch(`${API_BASE_URL}/trips`, { headers: getHeaders(token) });
      const data = await response.json();
  ...
  const fetchPayments = useCallback(async (userId = null, token = null) => {
    try {
  ...
  const fetchSupportTickets = useCallback(async (userId = null, token = null) => {
    try {
  ...
  const fetchNotifications = useCallback(async (userId = null, token = null) => {
    try {
  ...
  const markUnavailable = async () => {
    ...
    Alert.alert("Mark Unavailable", "Are you sure you want to mark yourself as unavailable for tomorrow?", [
      ...
      { text: "Yes, Unavailable", style: "destructive", onPress: async () => {
        try {
          setLoading(true);
          const response = await fetch(`${API_BASE_URL}/availability`, {
            method: 'POST',
  ...
  const startRoute = async () => {
    ...
    Alert.alert("Start Route", "Are you ready to start this route? GPS tracking will begin and passengers will be notified.", [
      { text: "Cancel", style: "cancel" },
      { text: "Start Route", onPress: async () => {
        try {
          setLoading(true);
  ...
  const renderDashboard = useMemo(() => (
    <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <View style={styles.contentPadding}>
        {showAvailabilityAlert && ( /* same as your code */ )}
        {showMorningConfirmAlert && currentTrip && ( /* same */ )}
        ...
        {currentRoute ? (
          <View style={styles.card} key={currentRoute._id}>
  ...
  ), [currentRoute, routeStarted, routeStops.length, dashboardStats, showAvailabilityAlert, showMorningConfirmAlert, currentTrip, unreadNotifications]);
    ...
    <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
      <View style={styles.contentPadding}>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search trips..." value={search} onChangeText={setSearch} placeholderTextColor="#999" />
        </View>
        ...
        {filteredTrips.length > 0 ? (
          filteredTrips.map((trip) => (
            <View key={trip._id} style={styles.card}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: 16, fontWeight: '700', color: '#333' }}>{trip.routeName || 'Route'}</Text>
                  <Text style={{ fontSize: 13, color: '#999', marginTop: 4 }}>{new Date(trip.createdAt).toLocaleDateString()}</Text>
                </View>
              ...
              <View style={{ flexDirection: 'row', marginTop: 8, gap: 16 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  ...
                  <Text style={{ fontSize: 13, color: '#666', marginLeft: 6 }}>{trip.passengers?.length || 0} passengers</Text>
                </View>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={styles.card}><Text style={styles.cardText}>No trips found</Text></View>
        )}
      </View>
    </ScrollView>
  ...
  return (
    <View style={styles.container}>
      ...
      {sidebarOpen && ( /* your sidebar code same */ )}
